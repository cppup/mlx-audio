"""Pure-MLX CAM++ speaker x-vector encoder for dots.tts.
  ...
  * ``kaldi_fbank(wav, sample_rate=16000) -> np.ndarray [T, 80]`` — a numpy
    reimplementation of the the original implementation ``Kaldi.fbank`` front-end (Povey window,
    ...
    upstream ``extract_speaker_fbank`` applies. Matches Kaldi to ~1e-5.
...
# the original implementation Kaldi constants (compliance/kaldi.py).
...
def _get_mel_banks(
    ...
    """numpy port of ``the original implementation.compliance.kaldi.get_mel_banks`` (vtln_warp=1).
...
def kaldi_fbank(wav, sample_rate: int = 16000) -> np.ndarray:
    ...
    Reproduces ``the original implementation.compliance.kaldi.fbank`` with the dots.tts speaker
...
class _Conv1d:
    ...
    Conv1d ``padding``); ``stride`` / ``dilation`` mirror the expected behavior.
    """
...
def _seg_pooling(x: mx.array, seg_len: int = 100) -> mx.array:
    ...
    sliced back to ``T`` — reproducing ``F.avg_pool1d(..., ceil_mode=True)``
