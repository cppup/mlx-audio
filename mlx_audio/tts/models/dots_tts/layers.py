...
Weight tensors are transposed to MLX layout at load
...
def hp_matmul(x: mx.array, w_t: mx.array) -> mx.array:
    ...
    Used on the *encode* path, whose deep stack
...
class Conv1d:
    ...
    reductions (see ``hp_matmul``) instead of ``mx.conv1d``
...
# Weights are stored in the expected layout (``Linear.weight`` is ``[out, in]``) and applied
# as ``x @ W.T``. The ``hp`` flag routes the activation@2-D-weight matmuls through the
# fp32 ``hp_matmul`` reduction ``hp=False``
# uses MLX's fast (~tf32) matmul / SDPA for the runtime path.
# --------------------------------------------------------------------------- #


class Linear:
    """Affine map ``x @ W.T (+ b)`` with weight ``[out, in]``.
    ...
    def __init__(self, weight: mx.array, bias: mx.array | None = None):
        self.weight = weight  # [out, in] (the expected layout)
...
class RMSNorm:
    ...
    Mirrors RMSNorm (eps=None default -> ``finfo(fp32).eps``): the
    ...
    # nn.RMSNorm(dim) is constructed with eps=None, which it resolves to
    # ``the expected format.finfo(x.dtype).eps`` (fp32 -> 1.1920928955078125e-07). Confirmed
    ...
    def __init__(
        ...
        # weight defaults to ones until load_weights overrides it.
...
class MultiHeadAttention:
    ...
    def load_weights(self, weights: dict) -> None:
        ...
        ``w_k_norm_weight``. Weights are stored in ``[out, in]`` layout.
        """
