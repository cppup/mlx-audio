...
class _SLSTM:
    ...
    Gate order is i, f, g, o. Uses both ``bias_ih`` + ``bias_hh``. Weights
    per layer are stored in the expected layout: ``weight_ih`` ``[4H, in]``, ``weight_hh``
...
class AudioVAE:
    ...
    def encode(self, wav: mx.array) -> mx.array:
        ...
        x = self.pre_proj(x)
        # Return [B, 2*latent_dim, T] (channel-first), matching the expected latent format.
